...
int main() {
  ...
  /* Hero Checks */
  {
    {
      try {
        Hero h{"", Hero_Class::BARBARIAN, Hero_Species::HUMAN, 9000,
               abilities()};
      } catch (const std::runtime_error& e) {
    ...
    {
      Hero h{"Jaina Proudmoore", Hero_Class::WIZARD, Hero_Species::HALF_ELF,
             9000, abilities()};
      std::cout << h << std::endl;
      std::cerr << h << std::endl;
...

    {
      for (size_t i{0}; i < 10; ++i) {
        ...
        for (size_t j{0}; j < 10; j++) {
          Elite_Monster m{"Goblin Archer",
                          static_cast<unsigned>(rand() % 20 + 70),
                          static_cast<unsigned>(rand() % 6 + 1),
                          static_cast<unsigned>(rand() % 3 + 1)};
  ...
  /* Campaign Checks */
  {
...

    {
      ...
      auto h1 = make_shared<Hero>("Jaina Proudmoore", Hero_Class::WIZARD,
                                  Hero_Species::HALF_ELF, 400, abilities());
      auto h2 = make_shared<Hero>("Arthas", Hero_Class::PALADIN,
                                  Hero_Species::HUMAN, 1000, abilities());
      auto h3 = make_shared<Hero>("Thrall", Hero_Class::WARLOCK,
                                  Hero_Species::HALF_ORC, 20, abilities());
      auto h4 = make_shared<Hero>("Sylvanas", Hero_Class::ROGUE,
                                  Hero_Species::ELF, 100, abilities());
  ...
  /* Player Checks */
  {
...

    {
      ...
      auto h1p1{p1.create_hero("Jaina Proudmoore", Hero_Class::WIZARD,
                               Hero_Species::HALF_ELF, 400, abilities())};
      auto h2p1{p1.create_hero("Arthas", Hero_Class::PALADIN,
                               Hero_Species::HUMAN, 1000, abilities())};
      std::cout << p1 << std::endl;
      p1.print_campaign(std::cout) << std::endl;
      Player p2{"Jane", "Doe"};
      auto h1p2{p2.create_hero("Thrall", Hero_Class::WARLOCK,
                               Hero_Species::HALF_ORC, 20, abilities())};
      ...
      /*
       * to test remove_dead(), the following method in Player can be
       * implemented is is available in the reference implementation
       * void campaign_encounter_monster(unsigned id,Monster& m);
       * throws error if no campaign available, otherwise calls
       * encounter_monster of campaign
       */
  ...
  std::cout << "-------------------Task 1-------------------" << std::endl;
  {
    ...
    vm.push_back(make_shared<Elite_Monster>("Young Adult Blue Dragon", 24200,
                                            222, 100));
    {
      auto x{task1(vm)};
      for (auto& m : x)
  ...
  {
    shared_ptr<Hero> h1{std::make_shared<Hero>("Hero 1", Hero_Class::WIZARD,
                                               Hero_Species::HALF_ELF, 1500,
                                               abilities())};
    shared_ptr<Hero> h2{std::make_shared<Hero>("Hero 2", Hero_Class::WIZARD,
                                               Hero_Species::HALF_ELF, 1100,
                                               abilities())};
    shared_ptr<Hero> h3{std::make_shared<Hero>("Hero 3", Hero_Class::WIZARD,
                                               Hero_Species::HALF_ELF, 9000,
                                               abilities())};
    shared_ptr<Hero> h4{std::make_shared<Hero>("Hero 4", Hero_Class::WIZARD,
                                               Hero_Species::HALF_ELF, 1200,
                                               abilities())};
    ...
    vector<shared_ptr<Monster>> vm;
    auto cp{c.enter_dungeon(vm)};
    h2.reset();
    std::cout << "Wins: " << cp.first << ", Loss: " << cp.second << std::endl;

    for (size_t i{0}; i < 8; ++i) {
      vector<shared_ptr<Monster>> vm;
      vm.push_back(make_shared<Standard_Monster>("Goblin Archer", 110, 5));
      vm.push_back(make_shared<Elite_Monster>("Goblin Elite Archer", 410, 99,
                                              5));
      vm.push_back(make_shared<Standard_Monster>("Goblin Archer", 110, 5));
      vm.push_back(make_shared<Standard_Monster>("Goblin Archer", 110, 5));
      vm.push_back(make_shared<Standard_Monster>("Goblin Archer", 110, 5));
  ...
  {
    shared_ptr<Hero> h1{std::make_shared<Task3>("Jaina Proudmoore",
                                                Hero_Class::WIZARD,
                                                Hero_Species::HALF_ELF, 9000,
                                                abilities())};

    shared_ptr<Hero> h2{std::make_shared<Task3>("Jaina Proudmoore2",
                                                Hero_Class::WIZARD,
                                                Hero_Species::HALF_ELF, 9000,
                                                abilities())};

    for (size_t i{0}; i < 10; ++i) {
      auto m{make_shared<Standard_Monster>("Goblin Archer", 110, 5)};
      h1->fight(*m);
      auto m2{make_shared<Standard_Monster>("Goblin Archer", 110, 5)};
      h2->fight(*m2);
  ...
  {
    shared_ptr<Hero> h1 = make_shared<Hero>("Jaina Proudmoore",
                                            Hero_Class::WIZARD,
                                            Hero_Species::HALF_ELF, 400,
                                            abilities());
    shared_ptr<Hero> h2 = make_shared<Hero>("Arthas", Hero_Class::PALADIN,
                                            Hero_Species::HUMAN, 1000,
                                            abilities());
    shared_ptr<Hero> h3 = make_shared<Hero>("Thrall", Hero_Class::WARLOCK,
                                            Hero_Species::HALF_ORC, 20,
                                            abilities());
    shared_ptr<Hero> h4 = make_shared<Hero>("Sylvanas", Hero_Class::ROGUE,
                                            Hero_Species::ELF, 100,
                                            abilities());
    shared_ptr<Hero> h5 = make_shared<Hero>("Jaina Elf", Hero_Class::WIZARD,
                                            Hero_Species::ELF, 400,
                                            abilities());
    Super_Task st{{h1, h2, h3, h4, h5}};
    {
      auto x{st.filtering({})};
      for (auto& h : x)
        std::cout << *h << std::endl;
    ...
    {
      auto x{st.filtering({{'r', Hero_Class::WIZARD}})};
      for (auto& h : x)
        std::cout << *h << std::endl;
    }
    std::cout << "-----------------------------------------------" << std::endl;
    ...
    {
      auto x{st.filtering({{'P', Hero_Class::WIZARD},
                           {'y', Hero_Class::ROGUE}})};
      for (auto& h : x)
        std::cout << *h << std::endl;
    }
    ...
    {
      auto x{st.filtering({{'o', Hero_Class::WIZARD},
                           {'X', Hero_Class::ROGUE}})};
      for (auto& h : x)
        std::cout << *h << std::endl;
    }
